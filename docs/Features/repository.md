...
Creating/Updating entities should be done using the CreateOrUpdateAsync method. Simple queries are possible with the methods GetOne, GetAll and GetPage.

As with services, repositories can also be extended by the developer and automatically registered with the DI container.
...
Add, AddRange, Attach, Update and UpdateRange are repository's extension methods for the DbSet's methods and can be used by the developers when doing multiple database operations. 
